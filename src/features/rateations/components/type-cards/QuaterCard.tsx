...
interface QuaterCardProps {
  breakdown: {
    due: KpiBreakdown;
    paid: KpiBreakdown;
    residual: KpiBreakdown;
  };
  saving: number; // Already in EUR from useQuaterSaving
  loading?: boolean;
}

function extractQuaterData(breakdown: KpiBreakdown) {
  return breakdown.find(b => b.type_label === 'Rottamazione Quater')?.amount_cents ?? 0;
}

export function QuaterCard({ breakdown, saving, loading = false }: QuaterCardProps) {
  const dueCents = extractQuaterData(breakdown.due);
  const paidCents = extractQuaterData(breakdown.paid);
  const residualCents = extractQuaterData(breakdown.residual);
  ...
  return (
    <div className="rounded-lg border bg-card p-4 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-semibold text-sm">Rottamazione Quater</h3>
      </div>
