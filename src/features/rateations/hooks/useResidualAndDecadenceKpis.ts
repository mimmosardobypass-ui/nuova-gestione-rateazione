...
export function useResidualAndDecadenceKpis() {
  ...
  const [overdueEffectiveEuro, setOverdueEffectiveEuro] = React.useState(0);
  ...
  const load = React.useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      const [resid, dec, overdueEff] = await Promise.all([
        fetchResidualEuro(signal),
        fetchDecadenceDashboardEuros(signal),
        fetchOverdueEffectiveEuro(signal),
      ]);
      ...
      setOverdueEffectiveEuro(overdueEff || 0);
    } catch (e: any) {
