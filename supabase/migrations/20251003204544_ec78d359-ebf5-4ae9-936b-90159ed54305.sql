...
SELECT
  ...
  -- Fallback RLS-safe per residuo PagoPA (amount in euro → cents)
  COALESCE(
    l.pagopa_residual_at_link_cents,
    (SELECT COALESCE(SUM((i.amount * 100)::bigint), 0)::bigint
     FROM v_installments_status i
     WHERE i.rateation_id = l.pagopa_id
       AND i.is_paid = FALSE)
  ) AS residuo_pagopa_at_link_cents,

  -- Fallback RLS-safe per totale RQ (amount in euro → cents)
  COALESCE(
