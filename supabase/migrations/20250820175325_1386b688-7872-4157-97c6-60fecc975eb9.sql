...
SELECT
  ...
  COALESCE(SUM(transferred_amount_cents), 0)                                     AS transferred,
  COALESCE(SUM(residual_at_decadence_cents - COALESCE(transferred_amount_cents,0)), 0) AS net_to_transfer
FROM rateations
