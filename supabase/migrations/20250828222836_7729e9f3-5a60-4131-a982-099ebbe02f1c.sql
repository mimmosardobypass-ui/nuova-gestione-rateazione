...
-- This is the definitive fix for KPI consistency issues
...
SELECT 
  ...
  -- PagoPA KPIs from separate view
  COALESCE(pk.unpaid_overdue_today, 0) AS unpaid_overdue_today,
  COALESCE(pk.unpaid_due_today, 0) AS unpaid_due_today,
  COALESCE(pk.max_skips_effective, 8) AS max_skips_effective,
