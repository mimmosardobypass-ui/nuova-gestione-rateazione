-- Migration: Add F24 Recovery Window to v_rateations_list_ui
-- Purpose: Extend view with f24_days_to_next_due for server-side F24 at-risk filtering
-- Non-breaking: adds nullable field, no existing logic modified

-- Drop and recreate v_rateations_list_ui with new field
...
WITH inst AS (
  SELECT 
    ...
    -- NEW: Calculate next unpaid F24 due date for recovery window
...
SELECT
  ...
  -- NEW: F24 Recovery Window field (days until next unpaid due date)
  CASE 
