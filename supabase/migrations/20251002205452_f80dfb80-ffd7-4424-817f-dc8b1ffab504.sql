-- Fix definitivo parsing robusto ID RQ con jsonb_array_elements_text()
-- Sostituisce il loop in pagopa_link_rq_v2 per eliminare definitivamente
-- l'errore "invalid input syntax for type bigint: ""14"""
...
DECLARE
  ...
  v_rq_txt text;  -- << nuova variabile per parsing robusto
  v_rq_id bigint;
  v_owner_uid uuid;
  v_is_pagopa boolean;
  v_is_rq boolean;
...
BEGIN
  ...
  -- 🔒 PARSING ROBUSTO: jsonb_array_elements_text() + validazione regex
  FOR v_rq_txt IN
    SELECT value
    FROM jsonb_array_elements_text(payload->'rq_ids')  -- << estrae testo pulito senza virgolette
  LOOP
    -- Rimuove spazi e valida formato numerico
    v_rq_txt := btrim(v_rq_txt);
    IF v_rq_txt = '' OR v_rq_txt !~ '^[0-9]+$' THEN
      RAISE EXCEPTION 'ID RQ non numerico: %', v_rq_txt;
    END IF;
    
    -- Cast sicuro a bigint (ora non può fallire)
