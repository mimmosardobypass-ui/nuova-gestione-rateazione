-- Phase 1: Create atomic RPC function for debt migration
-- This replaces the separate UPDATE + UPSERT operations with a single atomic transaction
...
-- Enhanced RLS policies for rateation_debts
-- Allow execution of migration function
CREATE POLICY "Users can execute migration function" 
ON public.rateation_debts 
FOR ALL 
USING (EXISTS (
  SELECT 1 FROM public.rateations r 
  WHERE (r.id = rateation_debts.rateation_id OR r.id = rateation_debts.target_rateation_id) 
  AND r.owner_uid = auth.uid()
));

-- Create rollback function for migration undo
