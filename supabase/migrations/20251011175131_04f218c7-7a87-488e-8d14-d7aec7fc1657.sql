-- ============================================================================
-- Fix definitivo: Periodo su installments.due_date per tutte le RPC Stats
-- Root cause: filtro su rateations.created_at escludeva rateazioni con rate
--             in scadenza fuori dal periodo di creazione
-- Soluzione: tutte le RPC ora filtrano su i.due_date per includere rateazioni
--            in tutti i periodi in cui hanno rate attive
-- ============================================================================

-- 1) Helper functions per normalizzazione parametri
...
-- 2) get_filtered_stats: periodo su due_date
...
BEGIN
  ...
  filtered_rateations AS (
    ...
    WHERE r.id IN (SELECT id FROM rateations_in_period)
      AND (
...
-- 3) stats_per_tipologia_effective: periodo su due_date
...
-- 4) get_residual_detail: periodo su due_date
...
BEGIN
  ...
  JOIN v_rateation_type_label vtl ON vtl.id = r.id
  WHERE 
    r.id IN (SELECT id FROM rateations_in_period)
...
-- 5) Indice performance per filtro su due_date
...
COMMENT ON FUNCTION public.get_filtered_stats IS 
'Stats globali con filtro periodo su installments.due_date. Una rateazione appare se ha almeno 1 rata nel periodo. Anti-doppio-conteggio F24↔PagoPA.';

COMMENT ON FUNCTION public.stats_per_tipologia_effective IS 
'Stats per tipologia usando v_rateation_type_label (PAGOPA prioritario su F24). Filtro periodo su installments.due_date. Esclude residui/ritardi F24↔PagoPA linkate.';

COMMENT ON FUNCTION public.get_residual_detail IS 
'Dettaglio rateazioni con residuo > 0. Filtro periodo su installments.due_date invece di created_at.';