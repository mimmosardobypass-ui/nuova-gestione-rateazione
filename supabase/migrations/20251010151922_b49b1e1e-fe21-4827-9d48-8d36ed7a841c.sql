...
-- 2. Mapping corretto tipologie (PAGOPA, Riammissione Quater)
-- 3. Gestione 'Riam.Quater' senza spazio
  ...
  mapped AS (
    SELECT
      CASE
        ...
        WHEN UPPER(b.type_name) LIKE '%ROTTAMAZIONE%' AND UPPER(b.type_name) LIKE '%QUATER%' THEN 'Rottamazione Quater'
        WHEN UPPER(REPLACE(b.type_name, '.', ' ')) ~ '(RIAM|RIAMM).*(QUATER)' THEN 'Riammissione Quater'
        ELSE 'Altro'
      END AS tipo,
      b.*
    FROM base b
