-- ============================================================================
-- FASE 1: PULIZIA COMPLETA - Elimina TUTTI gli overload precedenti
-- ============================================================================
-- Questo garantisce che Postgres non abbia ambiguità nella scelta della funzione
...
-- ============================================================================
-- FASE 2: UNA SOLA RPC con signature (text, text[], text)
-- ============================================================================
-- Cast sicuro con messaggi di errore chiari - Nessuna ambiguità possibile
...
begin
  ...
  foreach v_idtext in array p_rq_ids loop
    ...
    -- Return the link info using RETURN QUERY SELECT
...
-- ============================================================================
-- FASE 3: Crea RPC get_rq_available_for_pagopa se mancante
-- ============================================================================

create or replace function public.get_rq_available_for_pagopa(
  p_pagopa_id bigint
)
returns table (
  id bigint,
...
as $$
begin
  -- Verifica ownership della PagoPA
  if not exists(
    select 1 from rateations 
    where rateations.id = p_pagopa_id 
      and rateations.owner_uid = auth.uid()
  ) then
    raise exception 'Access denied to PagoPA %', p_pagopa_id;
  end if;

  -- Restituisce tutte le RQ dell'utente che:
  -- 1. Sono di tipo Riammissione Quater (is_quater = true)
  -- 2. Non sono decadute
  -- 3. NON sono già collegate attivamente alla PagoPA specificata
  return query
  select 
    r.id,
    r.number,
