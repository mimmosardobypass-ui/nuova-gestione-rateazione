-- Fix get_kpi_due_by_type to match UI filter "active_with_pending_decayed"
-- Problem: Function includes 'completata'/'INTERROTTA', excludes all 'decaduta'
-- Solution: Include active + F24 decadute with residuo > 0, exclude completata/interrotta
...
COMMENT ON FUNCTION public.get_kpi_due_by_type IS 'KPI breakdown: total due by type - matches UI filter "active_with_pending_decayed" (includes active + F24 decadute with residuo > 0)';